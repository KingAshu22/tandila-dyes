  ...
  },
  {
    ...
    header: ({ column }) => (
      <span
        ...
        className="flex items-center gap-1 cursor-pointer"
      >
        Balance Amt. <ArrowUpDown className="ml-2 h-4 w-4" />
      </span>
    ),
  },
  {
    ...
    header: "Actions",
    cell: ({ row }) => {
      const { invoiceNo } = row.original;
      return (
        <div className="flex gap-2">
          <Button
            className="bg-green-800"
            onClick={() => window.open(`/invoice/${invoiceNo}`)}
          >
            <Eye className="w-5 h-5" />
          </Button>
          <Button
            className="bg-blue-800"
            onClick={() => window.open(`/invoice/edit/${invoiceNo}`)}
          >
            <Pencil className="w-5 h-5" />
          </Button>
          <OutDateModal batchNo={invoiceNo} />
          <AlertDialog>
            <AlertDialogTrigger>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Do you really want to delete entry with Batch No: {invoiceNo} cannot be
                  undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
